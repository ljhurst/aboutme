...
resource "aws_api_gateway_resource" "recent_runs" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  parent_id   = aws_api_gateway_rest_api.main.root_resource_id
  path_part   = "recent-runs"
}
...
resource "aws_api_gateway_method" "recent_runs_get" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.recent_runs.id
  http_method   = "GET"
  authorization = "NONE"
  request_parameters = {
    "method.request.querystring.limit" = false
...
resource "aws_api_gateway_method" "recent_runs_options" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.recent_runs.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "recent_runs_options" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.recent_runs.id
  http_method = aws_api_gateway_method.recent_runs_options.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_method_response" "recent_runs_options" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.recent_runs.id
  http_method = aws_api_gateway_method.recent_runs_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "recent_runs_options" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.recent_runs.id
  http_method = aws_api_gateway_method.recent_runs_options.http_method
  status_code = aws_api_gateway_method_response.recent_runs_options.status_code
...
resource "aws_api_gateway_integration" "recent_runs_lambda" {
  rest_api_id             = aws_api_gateway_rest_api.main.id
  resource_id             = aws_api_gateway_resource.recent_runs.id
  http_method             = aws_api_gateway_method.recent_runs_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  content_handling        = "CONVERT_TO_TEXT"
  uri                     = aws_lambda_function.recent_runs.invoke_arn
}
...
resource "aws_lambda_permission" "api_gateway_recent_runs" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.recent_runs.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.main.execution_arn}/*/GET/recent-runs"
}
